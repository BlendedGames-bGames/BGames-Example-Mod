...
public class CognitiveCategoryEffectsScreen extends HandledScreen<CognitiveCategoryMenu> {
	...
	protected void drawForeground(MatrixStack poseStack, int mouseX, int mouseY) {
		...
		Text points  = ScreenHelper.getPoints(BGamesLibraryTools.getPoints(BgamesLibrary.bgames_cognitive_name,player));
		...
		Text[] effectNamesFirstRow= {Text.translatable("gui.bgamesmod.cognitive.experience_name"),ModEffects.REACH_BOOST.getName(),ModEffects.PICKUP_BOOST.getName()};
		for(int i=0; i<3;i++){
	...
	private void effectCheck(){
		if(player.hasStatusEffect(ModEffects.REACH_BOOST) && reachButton!=null){
			reachButton.setOff();
		}else{
		...
		if(player.hasStatusEffect(ModEffects.PICKUP_BOOST) && pickupButton!=null){
			pickupButton.setOff();
		}else{
		...
		reachButton = new BGamesButton(x, y, BUTTONS_WIDTH, BUTTONS_HEIGHT, 0, 0, BUTTONS_HEIGHT+BUTTONS_OFFSET, REACH_BOOST_EFFECT_BUTTON_TEXTURE,BUTTONS_WIDTH,BUTTONS_TOTAL_HEIGHT,
				...
				new ButtonWidget.TooltipSupplier(){
					...
					public void onTooltip(ButtonWidget buttonWidget, MatrixStack matrices, int i, int j) {
						...
						String amplifier= "X";
						tooltip.add(Text.literal(ModEffects.REACH_BOOST.getName().getString()+" "+amplifier) );
